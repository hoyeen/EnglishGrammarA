...
## 16. 浏览器单词朗读增量（2026-09-23）

基于浏览器原生 Web Speech API 为点词浮窗提供单词发音能力，不增加后端接口、付费服务或第三方运行时依赖。

- **手动触发与排队控制**：浮窗标题下方提供“发音”按钮，不自动播放；播放中按钮变为“停止”，可手动中断。发起新播放前清空队列，避免连续点击造成堆叠。
- **音色匹配**：每次触发时读取系统声音列表，优先选择英国英语（`en-GB`）音色，回退至其他英语音色；声音列表暂不可用时设置 `lang="en-GB"` 交由浏览器自选。提示用户发音由浏览器引擎生成，不保证匹配上下文 IPA。
- **生命周期与安全防护**：通过 React key（包含单词标识）与 unmount hook 绑定浮窗生命周期。在切词、关闭浮窗或重新提交整句分析时，自动取消当前发音。
- **异常恢复与回调防篡改**：引入单调递增的播放会话 ID，防止已取消的旧播放异步回调（`onend` / `onerror`）误改新播放状态；设置 15 秒兜底定时器，防止播放卡死；不支持 Web Speech API 的环境提示“当前浏览器不支持发音”。
