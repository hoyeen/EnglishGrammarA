...
一个面向英语学习者的响应式单页应用：分析英文句子结构与中文含义，或单独翻译英文单词和短语。

## 单词／短语翻译

切换到“单词／短语翻译”选项卡，输入不超过 100 字符的英文内容，即可获得简洁的中文译法。常见多义词最多显示三个译法；短语优先返回自然的整体译法。该功能调用现有 DeepSeek 配置，与句子分析和点词查询独立；孤立输入缺少上下文，因此译法不一定适合某个特定句子。翻译接口每客户端每分钟最多 30 次。
...
| `DEEPSEEK_MODEL` | `deepseek-flash` | 分析、点词和短语翻译模型 |
...
端到端测试会拦截分析和翻译 API，不调用真实模型、不产生 AI 费用。
...
应用不建立用户输入数据库，也不在自己的业务日志中记录原文、翻译或模型原始输出。句子会发送给 DeepSeek API 完成分析；单词／短语翻译输入会发送给 DeepSeek API 获得中文译法；首次点词或缓存失效后，原句、选中词与位置也会发送给 DeepSeek 解释本句词义。词典服务只接收查询词，不接收原句。第三方服务可能依据其自身条款产生必要的合规或安全日志。限流使用经过摘要处理的客户端标识。
